...
interface IMainErrorNoticeProps {
  errors: any[];
}

function MainErrorNotice({ errors }: IMainErrorNoticeProps) {
  ...
  return (
    <div className="border-2 border-red space-x-6 rounded bg-red-400">
      <h4>There has been an error while submitting the form.</h4>
      ...
      <ul className="mb-2 text-xs list-disc">
        {errorValues.map((error, index) => {
          ...
          return <li key={Math.random().toString()}>{message}</li>;
        })}
      </ul>
    </div>
  );
